'''
Projeto IA - Detecção de Retinopatia Diabética
...
def treinar_modelo(epochs=25):
    ...
    # Listas para armazenar métricas e plotar depois
    ...
    for epoch in range(epochs):
        ...
        corretos_treino, total_treino = 0, 0 # Para acurácia de treino
        ...
        for batch in train_loader:
            ...
            # Cálculo de acurácia no batch
            _, previsoes = torch.max(outputs, 1)
            corretos_treino += (previsoes == labels).sum().item()
            total_treino += labels.size(0)
            
        media_train_loss = train_loss / passos_treino
        acuracia_treino = corretos_treino / total_treino
        print(f"Loss de Treino: {media_train_loss:.4f} | Acurácia de Treino: {acuracia_treino:.4f}")
        
        # --- VALIDAÇÃO ---
        model.eval() 
        val_loss, passos_val = 0, 0
        corretos_val, total_val = 0, 0 # Para acurácia de validação
        ...
        with torch.no_grad(): 
            for batch in val_loader:
                ...
                # Cálculo de acurácia no batch
                _, previsoes = torch.max(outputs, 1)
                corretos_val += (previsoes == labels).sum().item()
                total_val += labels.size(0)
                
        media_val_loss = val_loss / passos_val
        acuracia_val = corretos_val / total_val
        print(f"Loss de Validação: {media_val_loss:.4f} | Acurácia de Validação: {acuracia_val:.4f}")
        
        # Salvando no histórico para os gráficos
        historico_train_loss.append(media_train_loss)
        historico_val_loss.append(media_val_loss)
        historico_train_acc.append(acuracia_treino)
        historico_val_acc.append(acuracia_val)
    ...
    # --- PLOTAGEM DAS CURVAS DE LOSS E ACURÁCIA ---
    plt.figure(figsize=(14, 5))
    
    # Gráfico de Loss
    plt.subplot(1, 2, 1)
    plt.plot(range(1, epochs + 1), historico_train_loss, label='Treino')
    plt.plot(range(1, epochs + 1), historico_val_loss, label='Validação')
    ...
    # Gráfico de Acurácia
    plt.subplot(1, 2, 2)
    plt.plot(range(1, epochs + 1), historico_train_acc, label='Treino')
    plt.plot(range(1, epochs + 1), historico_val_acc, label='Validação')
    ...
    plt.tight_layout()
    plt.savefig('curvas_treinamento_transfer.png', dpi=300)
    print("\nGráficos de treinamento salvos como 'curvas_treinamento_transfer.png'.")
    plt.show()
...
def avaliar_modelo():
    ...
    # Nova ordem: colocando a classe 4 (proliferate) antes da 3 (severe)
    ordem_desejada = [0, 1, 2, 4, 3] 
    nomes_classes = ["healthy", "mild", "moderate", "proliferate", "severe"]
    ...
    # Adicionado o parâmetro 'labels' para forçar a nova ordem
    print(classification_report(todos_rotulos_reais, todas_previsoes, labels=ordem_desejada, target_names=nomes_classes))
    
    # Cálculo da Taxa Kappa de Cohen
    kappa = cohen_kappa_score(todos_rotulos_reais, todas_previsoes)
    print(f"================ MÉTRICA KAPPA DE COHEN ================")
    print(f"Kappa Score: {kappa:.4f}")
    
    # Adicionado o parâmetro 'labels' na matriz de confusão
    cm = confusion_matrix(todos_rotulos_reais, todas_previsoes, labels=ordem_desejada)
